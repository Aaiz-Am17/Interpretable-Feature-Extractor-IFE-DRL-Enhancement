...
#env = GymAdapter.make(env_name)
import retro
from retro.examples.discretizer import Discretizer
import numpy as np
from common.frame_stack import FrameStack
...
## This code is modified to visualize the attention mask in Interpretable Feature Extractor work
class RecorderWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if self.writer is not None and self.frames_written < ( 60 * 60 * 9) and att_mask is not None:
            if self.scale_factor is None:
                self.scale_factor = EMULATOR_REC_SCALE
                if observation.shape[0] <= 64 and observation.shape[1] <= 64:
                    self.scale_factor *= 2
            rec_observation = att_masking(observation, att_mask)
            rec_observation = cv2.resize(rec_observation, (observation.shape[1] * self.scale_factor, observation.shape[0] * self.scale_factor),
...
# Visualize the attention on the frame
def att_masking(frame, att):
    if att == None:
        return frame
    # If you are evaluating the CNN as in the paper, change it to (15,15)
    original_att = rescale(att.view(14,14))
    w = frame.shape[1]
    h = frame.shape[0]
    
    overlay = cv2.resize(np.array(original_att), (w,h), cv2.INTER_NEAREST)
    overlay = cv2.cvtColor(overlay, cv2.COLOR_GRAY2BGR)
    image_uint8 = (overlay * 255).astype(np.uint8)
    bgr_image = cv2.addWeighted(frame, 0.6, np.zeros_like(frame), 0.8, 0)
    final_frame = cv2.addWeighted(bgr_image, 1, image_uint8, 1, 1)
    final_frame = cv2.resize(final_frame, (w,h) ,fx=0, fy=0, interpolation = cv2.INTER_NEAREST)
    return final_frame

# Rescale the attention mask to have consistent visualization
def rescale(tensor):
...
def create_atari_test_env(config):
    ...
    env = RecorderWrapper(env, fps=BASE_FPS_ATARI // config.frame_skip, save_dir=config.save_dir, label=config.env_name[4:], record_every=config.record_every)
    env = NoopResetEnv(env, 0, noop_max=30)
