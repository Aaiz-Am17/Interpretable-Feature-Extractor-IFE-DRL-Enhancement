...
class Rainbow:
    ...
    def __init__(self, env, args: SimpleNamespace) -> None:
        ...
        if self.prioritized_er:
            self.buffer = PrioritizedReplayBuffer(args.burnin, args.buffer_size, args.gamma, args.n_step, args.parallel_envs, use_amp=self.use_amp, is_test=self.test)
        else:
            self.buffer = UniformReplayBuffer(args.burnin, args.buffer_size, args.gamma, args.n_step, args.parallel_envs, use_amp=self.use_amp, is_test=self.test)
    ...
    def act(self, states, eps: float):
        ...
        with torch.no_grad():
            with autocast(enabled=self.use_amp):
                states = prep_observation_for_qnet(torch.from_numpy(np.stack(states)), self.use_amp, self.test)
    ...
    def load(self, path):
        ...
        self.q_policy.load_state_dict(checkpoint['state_dict'])
        print(f"Game frame: {checkpoint['game_frame']}")
